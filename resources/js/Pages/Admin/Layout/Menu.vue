<template>
    ...
    <div class="flex-grow space-y-3 px-2 overflow-y-scroll">
        <div class="flex-1">
            <ul class="pt-2 space-y-1 text-sm mb-2">
                ...
                <li class="hover:bg-slate-50 hover:text-black rounded-[5px] px-2 truncate" :class="$page.url.startsWith('/canned-replies') ? 'bg-primary text-white' : ''">
    ...
    <Link href="/admin/addons" class="border-2 border-slate-300 text-sm rounded-[5px] mb-1 m-3 py-2 px-4 flex items-center justify-between cursor-pointer">
    ...
    <div class="flex items-center m-3 p-2 rounded-xl h-14 py-1 md:py-1 mt-2 space-x-4 justify-between bg-slate-50">
        <div class="flex space-x-2">
            ...
            <div>
                <h2 class="text-[15px] capitalize truncate w-[6em]">{{ user.first_name }}</h2>
                <span class="flex items-center space-x-1">
                    <span @click="openModal" class="text-sm hover:underline dark:text-gray-400 cursor-pointer">{{ $t('View profile') }}</span>
                </span>
            </div>
        </div>
        <Link href="/logout" class="hover:bg-slate-200 text-slate-700 hover:rounded-full w-[fit-content] p-2">
